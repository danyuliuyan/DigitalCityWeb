...
<div class="main-container" style="margin-top: 65px">
    ...
    <i class="fa fa-angle-double-left" aria-hidden="true" id="hideArrow" title="点击隐藏博物馆列表"></i>
    <i class="fa fa-angle-double-right" aria-hidden="true" id="showArrow" title="点击显示博物馆列表"></i>

    <div class="left-menu">
        <h3><i class="fa fa-graduation-cap" aria-hidden="true"></i>博物馆列表</h3>
        <ul class="university-list">
            <c:if test="${museumList.size()==0}">暂无数据</c:if>
            <c:forEach var="listItem" items="${museumList}">
                <li data-lng="${listItem.longitude}" data-lat="${listItem.latitude}" data-id="${listItem.id}" data-type="2" title="点击地图可查看详细信息"><a href="javascript:;">${listItem.name}</a></li>
            </c:forEach>
        </ul>
    </div>
    <%--右侧地图信息--%>
